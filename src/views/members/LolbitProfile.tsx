...
export function LolbitProfile({ member }: { member: Member }) {
  ...
  return (
    <main className="relative min-h-screen bg-[#060608] text-neutral-300 antialiased selection:bg-[#f97316]/30 overflow-x-hidden">
      ...
      {/* ══ TOP BAR STICKY TÁCTIL (DESCONGESTIONADA) ══ */}
      <header className="sticky top-0 z-30 w-full border-b border-white/[0.05] bg-black/40 backdrop-blur-md py-3.5 px-6 sm:px-12 flex items-center justify-between gap-4">
        {/* Izquierda: retorno estilo terminal nítido */}
        <Link
          href="/"
          className="group inline-flex items-center gap-2 font-mono text-[11px] tracking-widest text-neutral-400 transition-colors hover:text-[#f97316]"
        >
          <ArrowLeft className="h-3.5 w-3.5 text-neutral-600 transition-all group-hover:-translate-x-1 group-hover:text-[#f97316]" />
          <span>← BACK // GLL SECTOR ZERO</span>
        </Link>

        {/* Centro: estado de sesión / red con pulso esmeralda */}
        <span className="hidden items-center gap-2 font-mono text-[11px] tracking-widest md:inline-flex">
          <span
          ...
          />
          <span className={lanyardIsLive ? "text-emerald-400" : "text-neutral-500"}>
            NODE // {lanyardIsLive ? "ONLINE" : "STANDBY"}
          </span>
        </span>

        {/* Derecha: micro-cápsulas de vidrio técnico */}
        <div className="flex items-center gap-2">
          {/* Píldora ping con punto verde */}
          <span className="inline-flex items-center gap-2 rounded-full border border-neutral-800/80 bg-neutral-900/60 px-2.5 py-1 font-mono text-[11px] tabular-nums">
          ...
          {/* Píldora CRT interactiva */}
          <button
            onClick={toggleCrt}
            aria-pressed={crtScanlines}
            title="Alternar filtro CRT Scanlines"
            className="inline-flex items-center gap-2 rounded-full border border-neutral-800/80 bg-neutral-900/60 px-2.5 py-1 font-mono text-[11px] text-neutral-300 transition-colors hover:border-neutral-600"
          >
            <Tv className={`h-3 w-3 ${crtScanlines ? "text-[#f97316]" : "text-neutral-500"}`} />
            <span>CRT</span>
      ...
      <article className="relative z-10 mx-auto max-w-5xl px-6 pb-24 pt-20 sm:pt-24">
        ...
        {/* ══ ACENTO EDITORIAL HAIRLINE ══ */}
        <div className="mt-8">
          <p className="mb-1.5 font-mono text-[10px] tracking-widest uppercase text-amber-500/70">
            {"// SYS.ID: 0x7F · SPECTRUM ONLINE"}
          </p>
          <div className="relative h-[1px] w-full max-w-xl bg-gradient-to-r from-amber-500/80 via-orange-500/40 to-transparent">
            <span
              aria-hidden
              className="absolute -top-[1px] left-0 h-[3px] w-20 animate-pulse bg-gradient-to-r from-amber-400/90 via-orange-400/50 to-transparent"
            />
          </div>
        </div>

        {/* ══ SUB-BARRA HUD INTEGRADA (VISITOR HARDWARE AUDIT) ══ */}
        <section className="mt-16 mb-16 border-y border-neutral-800/50" aria-label="Visitor hardware telemetry">
          <div className="flex flex-col justify-between gap-3 py-3.5 font-mono lg:flex-row lg:items-center">
            <span className="flex shrink-0 items-center gap-2 text-[10px] font-bold tracking-widest uppercase text-[#f97316]">
              <span className="lolbit-dot h-1.5 w-1.5 rounded-full bg-[#f97316]" />
              VISITOR TELEMETRY // HARDWARE AUDIT
            </span>
            <div className="flex flex-wrap items-center gap-y-2 text-[11px]">
              <div className="border-r border-neutral-800/60 pr-5 mr-5">
                <span className="block text-[9px] uppercase tracking-widest text-neutral-600">OS</span>
                <span className="text-neutral-300">{telemetry.os}</span>
              </div>
              <div className="border-r border-neutral-800/60 pr-5 mr-5">
                <span className="block text-[9px] uppercase tracking-widest text-neutral-600">Display</span>
                <span className="text-neutral-300">
                  {telemetry.resolution} <span className="text-neutral-500">@{telemetry.pixelRatio}x</span>
                </span>
              </div>
              <div className="border-r border-neutral-800/60 pr-5 mr-5">
                <span className="block text-[9px] uppercase tracking-widest text-neutral-600">Cores</span>
                <span className="text-neutral-300">{telemetry.cores}</span>
              </div>
              <div className="border-r border-neutral-800/60 pr-5 mr-5">
                <span className="block text-[9px] uppercase tracking-widest text-neutral-600">Mem</span>
                <span className="text-neutral-300">{telemetry.memory}</span>
              </div>
              <div className="border-r border-neutral-800/60 pr-5 mr-5">
                <span className="block text-[9px] uppercase tracking-widest text-neutral-600">GPU</span>
                <span className="text-emerald-400/90 truncate inline-block max-w-[200px] align-bottom">{telemetry.gpu}</span>
              </div>
              <div>
                <span className="block text-[9px] uppercase tracking-widest text-neutral-600">Ping</span>
